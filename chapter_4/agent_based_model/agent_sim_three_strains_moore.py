...
@jit(nopython=True)
def agent_rules(grid, beta01, beta02, beta03, beta12, beta21, beta23, beta32, beta13, beta31, v1, v2, v3, prb, eps):
...
def sim_run(num_sus, num_s1, num_s2, num_s3, num_row, num_col, num_steps, beta01, beta02, beta03, beta12, beta21, beta23, beta32, beta13, beta31, v1, v2, v3, prb, eps, num_sim):
    ...
    i3_counts = np.zeros(num_steps)
    time_steps = np.zeros(num_steps)

    grid = set_grid(num_sus, num_s1, num_s2, num_s3, num_agents)

    for i in range(total_steps):
        agent_rules(grid, beta01, beta02, beta03, beta12, beta21, beta23, beta32, beta13, beta31, v1, v2, v3, prb, eps)
        if i % num_agents == 0:
            s_counts[int(i/num_agents)] = np.sum(grid == 1)
            i1_counts[int(i/num_agents)] = np.sum(grid == 2)
            i2_counts[int(i/num_agents)] = np.sum(grid == 3)
            i3_counts[int(i/num_agents)] = np.sum(grid == 4)
...
def main():
    ...
    parser.add_argument('num_s3', type=int)
    ...
    parser.add_argument('beta03', type=float)
    parser.add_argument('beta21', type=float)
    parser.add_argument('beta12', type=float)
    parser.add_argument('beta23', type=float)
    parser.add_argument('beta32', type=float)
    parser.add_argument('beta13', type=float)
    parser.add_argument('beta31', type=float)
    ...
    simulations = [pool.apply_async(sim_run, args=(num_sus, num_s1, num_s2, num_s3, num_row, num_col, num_steps, beta01, beta02, beta03, beta12, beta21, beta23, beta32, beta13, beta31, v1, v2, v3, prb, eps, i)) for i in range(n_sims)]
